...
(defun make-kripke (k fma)
  (let ((a-kripke (make-instance 'eezot-kripke)))
    (setf 
      ...
      (setf (gethash '**I_POOL** (kripke-list a-kripke))
 		    (intern (format nil "I_POOL" )))

      (setf (gethash '**POOLEX** (kripke-list a-kripke))
 		    (intern (format nil "POOLEX" )))
	...
	(lambda (fm val)
	  ...
	  (if (symbolp fm)
	    (unless (member fm '(true false **I_LOOP** **LOOPEX** **I_POOL** **POOLEX**))
...
(defun the-poolEx ()
  (call-fmla-id *PROPS* '**POOLEX**))

(defun the-ipool ()
  (call-fmla-id *PROPS* '**I_POOL**))
...
(defun PoolConstraints ()
  (format t "loopconstraints...")(force-output)
  (list
   `(impl ,(the-poolEx) (and (<= 0 ,(the-ipool)) (< ,(the-ipool) ,(kripke-k *PROPS*))))
   (list
    'iff
     (list 'and `(<= 0 ,(the-ipool)) `(<= ,(the-ipool) ,(1- (kripke-k *PROPS*))))    
     (cons 'and 
 	   (cons
 	    (the-poolEx)
 	    (loop for p in (kripke-prop *PROPS*) collect 	  
 		 `(iff ,(call *PROPS* p `(+ ,(the-ipool) 1)) ,(call *PROPS* p 0))))))))
...
(defun gen-futr ()
  ...
  (loop for i from -1 to (kripke-k *PROPS*) append
	(loop for fma in (kripke-futr *PROPS*) collect
	      (list 'iff  
		    (call *PROPS* fma (if (= i -1) `(- 1) i))  
		    (case (car fma)
		      ((next)
		       (call *PROPS* (second fma) (1+ i)))

		      ((until)
		       `(or ,(call *PROPS* (third fma) (if (= i -1) `(- 1) i))
			    (and ,(call *PROPS* (second fma) (if (= i -1) `(- 1) i))
				 ,(call *PROPS* fma (1+ i)))))

		      ((release)
		       `(and ,(call *PROPS* (third fma) (if (= i -1) `(- 1) i))
			     (or ,(call *PROPS* (second fma) (if (= i -1) `(- 1) i))
				 ,(call *PROPS* fma (1+ i))))))))))
  ...
  (loop for i from 0 to (1+ (kripke-k *PROPS*)) append
	(loop for fma in (kripke-past *PROPS*) collect
	      (list 'iff  
		    ...
		    (case (car fma)
		      ((yesterday)
		       (call *PROPS* (second fma) (if (= i 0) `(- 1) (1- i))))

		      ((since)
		       `(or ,(call *PROPS* (third fma) i)
			    (and ,(call *PROPS* (second fma) i)
				 ,(call *PROPS* fma (if (= i 0) `(- 1) (1- i))))))
  ...
  (let ((fma-list (append
    ...
    (cons
     ...
     `(impl ,(the-poolEx)
	    ,(cons 'and
		   (loop for fm in fma-list collect 	    
			`(iff ,(call *PROPS* fm (the-ipool)) ,(call *PROPS* fm `(- 1) )))))))))
...
(defun gen-evt-past ()
  ...
  (append 
   (list 
    `(impl ,(the-poolEx)
	   ,(cons 'and		    
		  (loop for fm in (kripke-past *PROPS*) when (member (car fm) '(since trigger)) collect
		       (case (car fm)
			 ((since)			       
			  `(impl ,(call *PROPS* fm 0)
				 (and 				     
				  (<= ,(intern (format nil "I_EVE_~s" (call-fmla-id *PROPS* fm))) ,(the-ipool))
				  (<= 0 ,(intern (format nil "I_EVE_~s" (call-fmla-id *PROPS* fm))))
				  ,(call *PROPS* (third fm) (intern (format nil "I_EVE_~s" (call-fmla-id *PROPS* fm)))))))
			 
			 ((trigger)			       
			  `(impl (not ,(call *PROPS* fm 0))
				 (and 
				  (<= ,(intern (format nil "I_EVE_~s" (call-fmla-id *PROPS* fm))) ,(the-ipool))
				  (<= 0 ,(intern (format nil "I_EVE_~s" (call-fmla-id *PROPS* fm))))
				  (not ,(call *PROPS* fm (intern (format nil "I_EVE_~s" (call-fmla-id *PROPS* fm)))))))))))))))
				 ...
				 (if (consp key)
				   ...
				   (if (member key (kripke-prop *PROPS*))
				     ...
				     (case key
				       ...
				       ((**POOLEX**) (format k ":extrapreds (( ~s ))~%" v))
				       ((**I_POOL**) (format k ":extrafuns (( ~s Int))~%" v))
