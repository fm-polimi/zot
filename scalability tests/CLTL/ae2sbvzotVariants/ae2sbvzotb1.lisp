...
;; until with bvult, instead of redor
;; t, nil, zot-false are not allowed as AP name. true= t and false=nil (or (!! t))
;; Without int2bv i-loop = 3 => i_loop = ...1000
(in-package :cl-user)
...
(format k "
(define-fun getbit ((x (_ BitVec ~A)) (index (_ BitVec ~A))) (_ BitVec 1)
	(bvredor (bvand x index)))~%~4T" bvSize bvSize)
		
(format k "
(define-fun ilBitmap ((x Int)) Bool
	(ite (= x i-loop)
		true
		false))~%")
(format k "
(assert (= i_loop (concat 
	#b0")
(loop for i from the-time downto 1 do 
(format k "
	(ite (ilBitmap ~A) #b1 #b0)"i))
(format k "
	#b0)))~%")
