...
;; This version doesn't have i_loop.
...
; 	((_ extract 0 0) (bvlshr x index)))~%" bvSize bvSize)
		(format k "
(define-fun loopConF ((x (_ BitVec ~A))) Bool ;; k + 1 ~~ i-loop" bvSize)
		(format k "
	(= ((_ extract ~A ~A) x)" (+ the-time 1) (+ the-time 1))
		(loop for i from 1 to (1- the-time) do
			(format k "
		(ite (= i-loop ~A) ((_ extract ~A ~A) x)" i i i))
		(format k "
		((_ extract ~A ~A) x)" the-time the-time)
		(loop for i from 1 to (1+ the-time) do
		(format k ")" ))

		(format k "
(define-fun loopConV ((x (_ BitVec ~A))) Bool ;; k ~~ i-loop - 1
	(and (loopConF x)" bvSize)
		(format k "
		(= ((_ extract ~A ~A) x)" the-time the-time)
		(loop for i from 1 to (1- the-time) do
			(format k "
			(ite (= i-loop ~A) ((_ extract ~A ~A) x)" i (1- i) (1- i)))
		(format k "
		((_ extract ~A ~A) x)" (1- the-time) (1- the-time))
		(loop for i from 1 to (+ the-time 2) do
		(format k ")" ))
...
(define-fun until ((fap (_ BitVec ~A)) (A (_ BitVec ~A)) (B (_ BitVec ~A))) Bool
	(and" bvSize bvSize bvSize)
		...
		(or (= #b0 ((_ extract ~A ~A) fap))
			(bvugt B" (1+ the-time) (1+ the-time) (1+ the-time) (1+ the-time) (1+ the-time) (1+ the-time)  (1+ the-time) (1+ the-time) (1+ the-time) (1+ the-time) (1+ the-time) (1+ the-time) bvSize)
		(loop for i from 1 to (1- the-time) do
			(progn
				(format k "
				(ite (= i-loop ~A) #b" i)
				(loop for zero from 1 to (1+ (- the-time i)) do (format k "0"))
				(format k "1")
				(loop for zero from 1 to i do (format k "0"))))
		(format k "
					#b01")
		(loop for zero from 1 to the-time do (format k "0"))
		(loop for i from 1 to (+ the-time 3) do (format k ")"))
