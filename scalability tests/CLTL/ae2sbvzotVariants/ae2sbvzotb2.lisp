...
;; This version has arrays with BV indexes.
			...
			(cond 
			      ...
			      ((null p) 	   
					...
					;(format t "TERM non-cons ~s~%" obj)
					  ...
					  ; (list q the-time)))
					  (list q (list '_ (intern (format nil "bv~A ~A" the-time logbvSize))))
					  ))
			 ...
			 (let* ( (symbolic-valuation-XY-points (get-symbolic-valuation-points partition)) 
			       ...
			       (loop for point1 in symbolic-valuation-points append
				     (loop for point2 in (remove point1 symbolic-valuation-points) append
					   ...
					   `( (iff ,(make-IPC-constraint '< 
					   	; (call *PROPS* point1 `(- i-loop 1))
					   	(if (atom (call *PROPS* point1 0)) point1 (list (first (call *PROPS* point1 0)) (list (list '_ 'extract (1- logbvSize) 0) (list 'bvsub 'i_loop (list '_ 'bv1 bvSize)))))
					   	; (call *PROPS* point2 `(- i-loop 1))
					   	(if (atom (call *PROPS* point2 0)) point2 (list (first (call *PROPS* point2 0)) (list (list '_ 'extract (1- logbvSize) 0) (list 'bvsub 'i_loop (list '_ 'bv1 bvSize)))))
					   	) 

						    ,(make-IPC-constraint '< (call *PROPS* point1 (kripke-k *PROPS*)) (call *PROPS* point2 (kripke-k *PROPS*))))
					       ; (iff ,(make-IPC-constraint '= (call *PROPS* point1 `(- i-loop 1)) (call *PROPS* point2 `(- i-loop 1))) 
					       (iff ,(make-IPC-constraint '= (if (atom (call *PROPS* point1 0)) point1 (list (first (call *PROPS* point1 0)) (list (list '_ 'extract (1- logbvSize) 0) (list 'bvsub 'i_loop (list '_ 'bv1 bvSize)))))
					        (if (atom (call *PROPS* point2 0)) point2 (list (first (call *PROPS* point2 0)) (list (list '_ 'extract (1- logbvSize) 0) (list 'bvsub 'i_loop (list '_ 'bv1 bvSize)))))) 
						     ,(make-IPC-constraint '= (call *PROPS* point1 (kripke-k *PROPS*)) (call *PROPS* point2 (kripke-k *PROPS*)))))))))))))
		    ...
		    (nconc 
			  ...
			  (loop for i from 0 to (1- bound) append
				`(
				       ...
				       (iff (= ,(list (if (atom (call *PROPS* key 0)) key (first (call *PROPS* key 0))) (list (list '_ 'extract (1- logbvSize) 0) (list 'bvsub 'i_loop (list '_ 'bv1 bvSize)))) ,i)
				       	(= ,(call *PROPS* key (kripke-k *PROPS*)) ,i))

				       (iff (and
						  (< ,i ,(list (if (atom (call *PROPS* key 0)) key (first (call *PROPS* key 0))) (list (list '_ 'extract (1- logbvSize) 0) (list 'bvsub 'i_loop (list '_ 'bv1 bvSize))))) 
						  (< ,(list (if (atom (call *PROPS* key 0)) key (first (call *PROPS* key 0))) (list (list '_ 'extract (1- logbvSize) 0) (list 'bvsub 'i_loop (list '_ 'bv1 bvSize)))) ,(+ i 1)))
					     (and
						   (< ,i ,(call *PROPS* key (kripke-k *PROPS*))) 
						   (< ,(call *PROPS* key (kripke-k *PROPS*)) ,(+ i 1))))))
			  
			  (list `(iff (= ,(list (if (atom (call *PROPS* key 0)) key (first (call *PROPS* key 0))) (list (list '_ 'extract (1- logbvSize) 0) (list 'bvsub 'i_loop (list '_ 'bv1 bvSize)))) ,bound) (= ,(call *PROPS* key (kripke-k *PROPS*)) ,bound)))
			  (list `(iff (> ,(list (if (atom (call *PROPS* key 0)) key (first (call *PROPS* key 0))) (list (list '_ 'extract (1- logbvSize) 0) (list 'bvsub 'i_loop (list '_ 'bv1 bvSize)))) ,bound) (> ,(call *PROPS* key (kripke-k *PROPS*)) ,bound)))

			  ; Build the periodicity of regions between (i_loop) and K+1
			  (loop for i from 0 to (1- bound) append
			   	`(
		  	       (iff (= ,(list (if (atom (call *PROPS* key 0)) key (first (call *PROPS* key 0))) (list (list '_ 'extract (1- logbvSize) 0) 'i_loop)) ,i) (= ,(call *PROPS* key (1+ (kripke-k *PROPS*))) ,i))
		   	       (iff (and
		   			  (< ,i ,(list (if (atom (call *PROPS* key 0)) key (first (call *PROPS* key 0))) (list (list '_ 'extract (1- logbvSize) 0) 'i_loop))) 
		   			  (< ,(list (if (atom (call *PROPS* key 0)) key (first (call *PROPS* key 0))) (list (list '_ 'extract (1- logbvSize) 0) 'i_loop)) ,(+ i 1)))
		   		     (and
		   			   (< ,i ,(call *PROPS* key (1+ (kripke-k *PROPS*)))) 
		   			   (< ,(call *PROPS* key (1+ (kripke-k *PROPS*))) ,(+ i 1))))))

			  (list `(iff (= ,(list (if (atom (call *PROPS* key 0)) key (first (call *PROPS* key 0))) (list (list '_ 'extract (1- logbvSize) 0) 'i_loop)) ,bound) (= ,(call *PROPS* key (1+ (kripke-k *PROPS*))) ,bound)))
			  (list `(iff (> ,(list (if (atom (call *PROPS* key 0)) key (first (call *PROPS* key 0))) (list (list '_ 'extract (1- logbvSize) 0) 'i_loop)) ,bound) (> ,(call *PROPS* key (1+ (kripke-k *PROPS*))) ,bound)))
...
(defun zot (the-time spec 
  ...
  (setq logbvSize (1+ (floor (log (+ the-time 2) 2))))
...
(define-fun getbit ((x (_ BitVec ~A)) (index (_ BitVec ~A))) (_ BitVec 1)
	((_ extract 0 0) (bvlshr x index)))~%~4T" bvSize bvSize)
		(format k "
	...
	(until fap ~A A))~%~%" bvSize bvSize (bvTrue bvSize))
		...
		(format k ";;;;;;Used propositions:~%")
				    ...
				    (maphash (lambda (key v) 
					       ...
					       (format k "(declare-fun ~A ((_ BitVec ~A)) ~{~A ~})~%" key logbvSize sig)
				    ...
				    (maphash (lambda (key v) 
						   ...
						   (case (car key)
						     ...
						     (progn (format k "(declare-fun ~A ((_ BitVec ~A)) " v logbvSize) (if (or (string= (format nil "~A" (first (int-or-real key))) "Real") (string= (format nil "~A" (first (int-or-real key))) "real")) (format k "Real)~%") (format k "Int)~%"))))
						     ((+ - * / mod)
						     (progn (format k "(declare-fun ~A ((_ BitVec ~A)) " v logbvSize) (if (or (string= (format nil "~A" (first (int-or-real key))) "Real") (string= (format nil "~A" (first (int-or-real key))) "real")) (format k "Real)~%") (format k "Int)~%"))))
						     ))))
