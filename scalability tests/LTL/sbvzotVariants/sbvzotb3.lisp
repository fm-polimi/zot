...
;; sbvzotMain -> eventuality is managed using reverse and +/-.
  ...
  (let ((formula (bvf (trio-to-ltl spec) (+ the-time 2))))
	...
	(progn 
	  ...
	  (time (progn
		...
		(format k "(concat")
			(loop for i from 0 to (- bvSize 1) do
			(format k " ((_ extract ~A ~A)x)" i i))
		(format k "))")
		(format k "~%
...
(format k "
		...
		(or (= #b0 ((_ extract ~A ~A) fap))
			(= (_ bv0 ~A) (bvand iloops (reverse (bvsub (bvand (bvadd (bvnot (reverse B)) (_ bv1 ~A)) (reverse B)) (_ bv1 ~A))))))))
		" (1+ the-time) (1+ the-time) (1+ the-time) (1+ the-time) (1+ the-time) (1+ the-time)  (1+ the-time) (1+ the-time) (1+ the-time) (1+ the-time) (1+ the-time) (1+ the-time) bvSize bvSize bvSize)
