...
; ------------------------------------------
;; sbvzot with bvuntil. classic until with word-level encoding.
;; without any binary addition/subtraction. 
;; (iloops = xnor Aps ...) is replaced with (iloops => xnor Aps ...)
...
(defun gen-futr ()
  ...
  (loop for fma in (kripke-futr *PROPS*) collect
	(case (car fma)
	  ...
	  ((alwf)
	 	(list 'alwf (first (call *PROPS* fma 0)) (first (call *PROPS* (second fma) 0)) (intern (format nil "zot-eve-~A" (first (call *PROPS* fma 0))))))

	  ((somf)
	 	(list 'somf (first (call *PROPS* fma 0)) (first (call *PROPS* (second fma) 0)) (intern (format nil "zot-eve-~A" (first (call *PROPS* fma 0))))))
      ...
      ((until)
		(list 'until (first (call *PROPS* fma 0)) (first (call *PROPS* (second fma) 0)) (first (call *PROPS* (third fma) 0)) (intern (format nil "zot-eve-~A" (first (call *PROPS* fma 0))))))

	  ((release)
		(list 'release (first (call *PROPS* fma 0)) (first (call *PROPS* (second fma) 0)) (first (call *PROPS* (third fma) 0)) (intern (format nil "zot-eve-~A" (first (call *PROPS* fma 0)))))))))
...
(defun the-big-formula (fma periodic-arith-terms gen-symbolic-val ipc-constraints bound)
  (append
   (nconc	   
  ...
  (let ((formula (bvf (trio-to-ltl spec) (+ the-time 2))))
	...
	(progn 
	  ...
	  (time (progn
		...
		(format k "(declare-fun iloops () (_ BitVec ~A))" bvSize)
		(format k "~%
...
(format k "
(define-fun eve ((eve-fap (_ BitVec ~A)) (x (_ BitVec ~A))) Bool
	(and
		(= ((_ extract ~A ~A) eve-fap) ((_ extract ~A ~A) x))" bvSize bvSize (1- bvSize) (1- bvSize) (1- bvSize) (1- bvSize))
(format k "
		(= ((_ extract ~A 0) eve-fap) (bvor ((_ extract ~A 0) x) ((_ extract ~A 1) eve-fap)))))
" the-time the-time (1+ the-time))
;<until3> classic

(format k "
(define-fun until ((fap (_ BitVec ~A)) (A (_ BitVec ~A)) (B (_ BitVec ~A)) (eve-fap (_ BitVec ~A))) Bool
	(and
		(eve eve-fap B)" bvSize bvSize bvSize bvSize)
(format k "
...
(format k "
		...
		(or (= #b0 ((_ extract ~A ~A) fap))
			(= (_ bv0 ~A) (bvand iloops (bvnot eve-fap))))))
...
(format k "
(define-fun release ((fap (_ BitVec ~A)) (A (_ BitVec ~A)) (B (_ BitVec ~A)) (eve-fap (_ BitVec ~A))) Bool
	(until (bvnot fap) (bvnot A) (bvnot B) eve-fap))~%" bvSize bvSize bvSize bvSize)
		(format k "
(define-fun since ((fap (_ BitVec ~A)) (A (_ BitVec ~A)) (B (_ BitVec ~A))) Bool" bvSize bvSize bvSize)
		(format k "
	(and
		(= ((_ extract 0 0) fap) ((_ extract 0 0) B) )")
		; (loop for i from 1 to (1+ the-time) do
		; 	(format k "
		; (= ((_ extract ~A ~A) fap) (bvor ((_ extract ~A ~A) B) (bvand ((_ extract ~A ~A) A) ((_ extract ~A ~A) fap))))" i i i i i i (1- i) (1- i)))
		(format k "
...
(format k "
(define-fun alwf ((fap (_ BitVec ~A)) (A (_ BitVec ~A)) (eve-fap (_ BitVec ~A))) Bool
	(until (bvnot fap) ~A (bvnot A) eve-fap))~%" bvSize bvSize bvSize (bvTrue bvSize))
(format k "
...
(format k "
(define-fun somf ((fap (_ BitVec ~A)) (A (_ BitVec ~A)) (eve-fap (_ BitVec ~A))) Bool
	(until fap ~A A eve-fap))~%~%" bvSize bvSize bvSize (bvTrue bvSize))
		...
		(format k ";;;;;;Used propositions:~%")
			...
			(loop for fma in (kripke-atomic-formulae *PROPS*) do
				(push (first (call *PROPS* fma 0)) (kripke-allsubf *PROPS*)))
			(format k "
(assert (and (= #b0 ((_ extract 0 0) iloops)) (= #b0 ((_ extract ~A ~A) iloops))))" (1+ the-time) (1+ the-time))
			(format k "
(assert (= ~A (bvor (bvnot iloops) (concat #b0 (bvand" (bvTrue bvSize))
			(loop for fma in (kripke-allsubf *PROPS*) collect
			  	(format k "~%~4T(bvxnor ((_ repeat ~A) ((_ extract ~A ~A) ~A)) ((_ extract ~A 1) ~A))" the-time (1+ the-time) (1+ the-time) (string-downcase fma) the-time (string-downcase fma)))
			(format k ") #b0))))~%")
			(format k "(assert (= (bvredor iloops) #b1))~%")
			(format k ";;;;;;The main formula is asserted to be true at the time instant 1:~%")
			(format k "(assert (= ((_ extract 1 1) zot-p1) #b1))~%")
			; (format k "(check-sat-using (then simplify solve-eqs (repeat bit-blast) sat) :print_model true)~%")
			; (format k "(check-sat-using (then simplify solve-eqs (repeat bit-blast) (! sat :asymm_branch false :elim_blocked_clauses true :resolution false :scc false)) :print_model true)~%")
			(format k "(check-sat-using (then simplify solve-eqs (! propagate-values :blast_eq_value true) (repeat bit-blast) (! sat :asymm_branch false :elim_blocked_clauses true :resolution false :scc false)) :print_model true)~%")
