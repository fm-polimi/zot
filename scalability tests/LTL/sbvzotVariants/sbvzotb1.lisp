...
;; in this version we have iloops (ls), and eventuality is handled by B > ls.
(in-package :cl-user)
(defpackage :sbvzot
  (:use :common-lisp 
	:trio-utils
  ...
  (let ((formula (bvf (trio-to-ltl spec) (+ the-time 2))))
	...
	(progn 
	  ...
	  (time (progn
		...
		(when (not loop-free) (format k "~%(declare-fun iloops () (_ BitVec ~A))" bvSize))
		(format k "~%
...
(define-fun until ((fap (_ BitVec ~A)) (A (_ BitVec ~A)) (B (_ BitVec ~A))) Bool
	(and" bvSize bvSize bvSize)
		...
		(or (= #b0 ((_ extract ~A ~A) fap))
			(bvugt B iloops))))
		" (1+ the-time) (1+ the-time) (1+ the-time) (1+ the-time) (1+ the-time) (1+ the-time)  (1+ the-time) (1+ the-time) (1+ the-time) (1+ the-time) (1+ the-time) (1+ the-time))))
(format k "
...
(assert (= ~A (bvor (bvnot iloops) (concat #b0 (bvand" (bvTrue bvSize))
					(loop for fma in (kripke-allsubf *PROPS*) do
						...
						(format k ") #b0))))~%")))
			(when (not loop-free) (format k "(assert (= (bvredor iloops) #b1))~%"))
			(format k "~%;;;;;;The main formula is asserted to be true at the time instant 1:~%")
			(format k "(assert (= ((_ extract 1 1) zot-p1) #b1))~%")
			(if loop-free
				(format k "(check-sat-using (then simplify solve-eqs (! propagate-values :blast_eq_value true) (repeat bit-blast) (! sat :asymm_branch false :elim_blocked_clauses true :resolution false :scc false)) :print_model ~A)~%" (if getLFmodel "true" "false"))
