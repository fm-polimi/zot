  ...
  (let ((a-kripke (make-instance 'ae2zot-kripke)))
    ...
    (labels (
      ...
      (let* ( (IPC-terms 
        ...
        (loop 
          while IPC-terms do
            ;; choose a new variable from IPC-terms to start a new partition
      ...
      ;; *** complete arithmetical data structures by adding all terms
      ;; *** (Xy/Yx) which are not defined in the formula. This is
      ;; *** done to define correctly LoopConstraints.
     ...
     (let ( (p (gethash obj (kripke-list kk))) 
       ...
       (cond 
         ...
         ((null p)                                                    
          (if (consp q)
              (if other
                  ;; if other is specified then call is evaluating a
                  ;; nested term. Therefore, inner terms should be
                  ;; marked with the time instant defined by other
                  (progn
                    (format t "Term: ~s~%" q)
...
(defun gen-futr1 ()
  ...
  (loop for i from 0 to (1- (kripke-k *PROPS*)) append       
        (loop for fma in (kripke-futr *PROPS*) ;KRIPKE-FUTR returns all sub-formulae that have future operators
              collect
              (list 'iff  
                    (call *PROPS* fma i)  
                    (case (car fma)
                      ((next after )    ;after is the same as next, except at the end of the model.
...
(defun gen-futr2 ()
  ...
  (loop for fm in (kripke-futr *PROPS*) collect
        (case (car fm)
          ...
          ;; unlike next, which is always false at the end of the
          ;; bound in BMC, after is always true at the end of the bound
          ((after)
...
(defun the-big-formula (fma loop-free no-loop periodic-arith-terms gen-symbolic-val ipc-constraints bound discrete-regions parametric-regions discrete-counters signals mtl-intervals init-signals)      
  (cons
   ...
   (nconc
    (gen-bool)      ;semantics of boolean operators
    (gen-futr1)     ;semantics of future operators (next, until, release) for each position
    (gen-futr2) ;semantics of future operators (next, until...) at the end of the model
    (gen-past1)                    ;same, for past operators
    (gen-past2)        
...
(defun zot (the-time ; the number of discrete locations we need
            spec 
                     &key 
                     (loop-free nil) 
                     (transitions nil)
  ...
  (let ((formula (deneg (trio-to-ltl spec)))) ; deneg means all the negations are pushed in to the literals
                                        ; trio-to-ltl removes syntactic sugar from trio form
                          ...
                          (to-smt-dialect 
                           (nconc (list 'and)                                               
                                  ...
                                  (if (and trans negate-transitions)
                                    ...
                                    (deneg trans)))
                           smt-lib))) ;specify which SMT language to use for output
