...
(defun LTL-formulap (f) 
  (and (consp f)        
   (in (car f) '(not and or iff next until release since trigger zeta yesterday futr lasts withinf past Zpast lasted Zlasted withinp Zwithinp))))	;possibly wrong
         ...
         (if (or 
              ...
              (arith-itemp fm)) ; if fm is an uninterpreted RELATION
               ...
               (case (car fm)
                ...
                ((next until release futr lasts withinf) 				    ;metric semantic added only for finite words
                 (push fm (kripke-futr a-kripke)))
                ((yesterday zeta since trigger past Zpast lasted Zlasted withinp Zwithinp)  ;metric semantic added only for finite words
...
;semantic changes for finite word metric operators
		...
		((lasts)	;works only for finite words
                `(and ,@(loop for j from 1 to (third fma) collect (call *PROPS* (second fma) (+ i j)))))
		
		((withinf)	;works only for finite words
...
;semantic changes for finite word metric operators
     ((past) ;works only for finite words
      `(not ,(call *PROPS* fm 0)))
     ((Zpast) ;works only for finite words
      (call *PROPS* fm 0))

     ((lasted) ;works only for finite words
      `(not ,(call *PROPS* fm 0)))
     ((Zlasted) ;works only for finite words
      (call *PROPS* fm 0))

     ((withinp) ;works only for finite words
      `(not ,(call *PROPS* fm 0)))
     ((Zwithinp) ;works only for finite words
      (call *PROPS* fm 0))
...
;semantic changes for finite word metric operators
     ((past Zpast) ;works only for finite words
      `(iff ,(call *PROPS* fm i)
